...
class GPModel():
    def __init__(self, prior_file='Prior.csv', test_file='Test.csv', batch_size=10) -> None:
        self.kernel = gpflow.kernels.RationalQuadratic()  + gpflow.kernels.Matern12() + gpflow.kernels.Matern32() + gpflow.kernels.RationalQuadratic()
    ...
    def extract_x_data(self, df, prior=True):
        ...
        x4 = np.atleast_2d(x4).flatten().reshape(-1,1)
        x5 = np.atleast_2d(x5).flatten().reshape(-1,1)
        ...
        if prior:
            ...
            #Extracting the mean and std. dev
            self.x4_m = np.mean(x4)
            self.x4_std = np.std(x4,ddof=1)

            #Extracting the mean and std. dev for bl_test
            self.x5_m = np.mean(x5)
            self.x5_std = np.std(x5,ddof=1)

        #Standardising the input features
        x1_s = (x1 - self.x1_m)/self.x1_std
        x2_s = (x2 - self.x2_m)/self.x2_std
        x3_s = (x3 - self.x3_m)/self.x3_std
        x4_s = (x4 - self.x4_m)/self.x4_std
        x5_s = (x5 - self.x5_m)/self.x5_std
        
        x_s = np.vstack((x1_s.flatten(),x2_s.flatten(), x3_s.flatten(),x4_s.flatten(), x5_s.flatten())).T
    ...
    def update_model(self, data_point_idx):
        ...
        self.opt.minimize(self.model.training_loss, self.model.trainable_variables, options=dict(maxiter=10000),method="L-BFGS-B")
    ...
    def calculate_r2(self):
        ...
        y_pred = np.where((y_pred <= 0) | np.isnan(y_pred) | np.isinf(y_pred),  0.00002, y_pred)
    ...
    def save_predictions(self, predictions_path):
        ...
        np.savetxt(predictions_path, y_pred, delimiter=",", header="Predictions", comments='', fmt='%.6f')
